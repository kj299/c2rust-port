...
#   3. differential       : diff_run over matrix-scalar.json (the modules ported
...
"$PY" "$KIT/harnesses/differential/diff_run.py" \
    ...
    --matrix "$HERE/oracle/matrix-scalar.json" --ledger "$HERE/DIVERGENCES.md"
...
( cd "$KIT"   # ingest verifies report provenance against THIS repo's HEAD
  "$PY" harnesses/progress/progress.py --file "$HERE/progress.json" \
      ingest --diff-json "$HERE/reports/alloc-node.json" "$HERE/reports/scalar-parse.json"
  "$PY" harnesses/progress/progress.py --file "$HERE/progress.json" show )

echo ""
echo "===== cJSON PORT GATE COMPLETE — modules 1-2 differential-green ====="
