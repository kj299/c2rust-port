...
hand-written (LESSONS #17, mechanized; LESSONS #21).
...
with it. LESSONS #17 made probe-first a convention; conventions decay without a
control (LESSONS #13). This harness is the control:
  ...
  * `run`    executes every probe against the C oracle and pins the observed
    (rc, stdout) — byte-faithfully (LESSONS #14) — under a fingerprint.
...
Fail-closed (LESSONS #6): zero probes is an error, not a pass (a transcript
that pinned nothing is the 0-of-0 audit again — LESSONS #18); a probe that
...
def load_probes(path):
    ...
    if not isinstance(raw, list) or not raw:
        ...
        # (LESSONS #18): NOTHING-TO-PROBE is a failure, never a pass
...
def run_oracle(oracle, probe, timeout):
    ...
    try:
        ...
        # fail closed: a hang cannot be pinned as an expectation (LESSONS #6 —
...
def load_transcript(path):
    ...
    if not entries:
        raise ValueError(f"{path}: transcript pins NOTHING (LESSONS #18)")
...
def _rust_bytes(data: bytes) -> str:
    """A Rust byte-string literal, byte-faithful (LESSONS #14): printable
...
def _self_test():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        check("observed rc + stdout recorded",
              ...
              and e["rejects"]["rc"] == 1)
        check("non-UTF-8 stdout stored byte-faithfully (LESSONS #14)",
        ...
        # zero probes / duplicate ids fail closed (LESSONS #18)
        empt = os.path.join(d, "empty.json")
        json.dump({"module": "demo", "probes": []}, open(empt, "w"))
        check("zero probes is a FAILURE, not a pass (LESSONS #18)",
        ...
        check("oracle hang fails closed (LESSONS #6)",
