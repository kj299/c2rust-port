...
Why (LESSONS #46): `#![forbid(unsafe_code)]` on `core` is the first row of this
...
not see the gap either: the row named no harness, so until LESSONS #45 it was
...
not accept — LESSONS #44, #45):

  * the attribute in a COMMENT or doc comment. Both core crates this kit ships
    quote the attribute in their `//!` header, so a grep finds that line and
...
Exit:   0 = every crate forbids unsafe_code on every target root
        ...
        2 = a path is not a crate, or a crate has no target root — a check over
            nothing proves nothing and must not pass (LESSONS #18)
"""
...
def _self_test():
    ...
    with tempfile.TemporaryDirectory() as t:
        ...
        # 0-of-0 and missing inputs are errors, not passes (LESSONS #18).
