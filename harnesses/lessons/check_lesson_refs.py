#!/usr/bin/env python3
# KIT-IMPORT: from the lsof line of this kit (porting-kit/, vendored in kj299/lsof),
#   brought back by this kit's LESSONS #45 retrospective. Its history below is lsof's:
#   lsof entries are named "the lsof line's entry NNN", never as a citation, because
#   the two logs share #001-#005 and then diverge. Re-cited: lsof #033 -> #13.
...
motivated this check was silent for three days in the lsof line: commit 15d7a03
deleted the heading line of that log's `## 022.` from an append-only file while
leaving its body, which spliced entry 022's text onto the end of entry 021 and
left PLAYBOOK's Phase 5 citation of entry 22 pointing at nothing. No harness, and no markdown link
...
KIT_ROOT, since that is what a citation resolves *against* (LESSONS #13: a
recorded lesson is not a control until something fails without it. The lsof
line's entry 033 adds that the gap was described as "one citation" from a grep
before it was measured; a gate's reach and its CI trigger have to move together).
...
# matches a citation broken across a line, which is exactly how the lsof line's
# entry-22 one was written.
...
# "0 problems" in the lsof line on a two-member citation whose second member
# did not exist. The shapes (examples of SYNTAX only — they claim nothing):
#
...
# tree silently resolved to the wrong one with this checker green (the lsof
# line's entry 056). A checker that recognises one form of the thing it counts
# is blind to every other form, and blind in the direction that reads as
# success — the same shape as this kit's LESSONS #44 and #45.
...
# never visited stale (the lsof line's entry 057). `.jsonl` for the same
# reason: the decision sweep's ledger cites its lesson on every line, and
# the lsof line's first collision after it landed would have renumbered
# every file but that one (LESSONS #49).
...
#
#     @# ... is where citations go stale (LESSONS #NNN,
#     @# #NNN). The resolver ...
#
...
# listed the token the strict rules had not touched (the lsof line's entry
# 057). A marker
...
def _self_test():
    ...
    with tempfile.TemporaryDirectory() as root:
        ...
        # ...and so is a line of a JSON-lines ledger (LESSONS #49).
