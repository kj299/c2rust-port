...
And it audits every DECISION in a verdict function (LESSONS #48). The table is
one row per verdict by convention (LESSONS #16), and the table audit counts rows
...
COVERAGE_EXEMPT = {
    "harnesses/gate-mutation/mutate_gates.py":
        ...
        "(LESSONS #48), run over its own verdict functions, leaves only report "
...
# ------------------------------------------ one row per verdict (LESSONS #48) --
#
# The table audit above counts rows per harness. In the lsof line's copy of this
# kit (its entry 064) a call-must-be-code rule landed in the scanner's verdict
# function with a fixture and no row; the scanner already had two rows, so
# nothing could notice. So the sweep derives the verdicts itself.
#
...
# parsers. LESSONS #20 draws the same line — and says plumbing has bugs of its
# own, which this does not look for. A verdict moved into a helper escapes,
...
def load_ledger(kit_root):
    ...
    field is a line it cannot read (LESSONS #44)."""
...
def run_gates(kit_root, mutations, as_json=False, check_coverage=True,
    ...
    # LESSONS #48: an unpinned decision passes only if the ledger names it, and
    ...
    if as_json:
        ...
        if new:
            print(f"\nUNPINNED: {len(new)} decision mutant(s) in a verdict function "
                  f"leave the self-test green (LESSONS #48). Add a fixture that "
...
# LESSONS #48: a verdict function with five decisions. `x is None` and `x < 0`
...
# shape lsof's missed verdict had), a comprehension filter, a `not` on its own,
...
def _self_test():
    ...
    with tempfile.TemporaryDirectory() as root:
        ...
        # not the verdict → hard error. Unpinned until LESSONS #48's sweep was
        # run over this file.
        ...
        # directly; run over this file, LESSONS #48's decision sweep found that
    ...
    # LESSONS #48: one row per verdict, enforced. Every decision in a verdict
    ...
    with tempfile.TemporaryDirectory() as root:
        ...
        check("a misspelled field is a hard error, not a dropped line (LESSONS #44)",
              exits(lambda: run_gates(kit, [row])))
        ...
        # breaks two things pins only their union (LESSONS #16).
        for label, bad in [
